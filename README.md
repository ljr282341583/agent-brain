...
名字对不上时(本地改名、错别字),skill 会**先近似扫描并询问**,确认不是重复才新建——笔记不会悄悄分裂。
...
- 项目仓库一律用**原来的项目文件夹**;要最新代码 = 在该文件夹 `git fetch`/`pull` 确认,
  **不另建克隆副本**(确需新克隆,先问用户目标路径)。收尾的**项目笔记**只写进大脑仓库
  对应项目;机器登记 `machines.md`、`inbox.md` 等系统文件照常写。
- 讲操作理由时,把**成文约定**与**个人推断**分开标明;不得把自己的推断说成"约定"。
```
...
- DECISIONS 被后条取代时,在旧条目**末尾追加** `> ⚠️ 已被 <日期>「<新条目标题>」取代` 标注——这属于追加,不算改写历史,由做出新决策的一方顺手标注
