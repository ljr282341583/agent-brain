...
## 2026-09-25 · 手机访问链路内置到桌面端（中继随 App 起停）

- **决定**：手机连电脑不再依赖"另开一个命令行窗口跑配对脚本"。中继改由**桌面端主进程内**提供
  （`ds-harness-desktop` v0.3.8）：桌面端启动即就绪、退出即关闭；配对二维码入口放在托盘菜单与
  窗口菜单（「手机访问 → 显示配对二维码」）。本仓库的 `tools/dsh-mobile-pair.mjs` /
  `dsh-mobile-relay.mjs` 与 `启动手机访问.cmd` **保留**为独立 / 开发用路径，不再当日常入口。
- **为什么**：那个窗口必须一直开着，用户关掉手机就断，容易被误读成"项目坏了"；而中继本身**不持有
  任何凭据**（认证仍由官方令牌 + 30 天 cookie 负责），只做 `Host`/`Origin` 改写——放进桌面端进程里
  既省掉一个步骤，又天然随 App 生死、不留孤儿进程。
- **否决**：① 中继做成开机自启的后台服务（多一个常驻进程、与桌面端生命周期不同步）；
  ② 做成桌面端的第二个子进程（多一个进程要管，且与手机端那份实现有漂移）；
  ③ 让桌面端直接绑尾网地址对外（违反本项目"不绕厂商安全闸"的约定，且官方 CLI 拒绝非回环绑定）。
- 证据：桌面端 `app/src/relay.js` / `tailnet.js` / `main.js`；手机端 `tools/dsh-mobile-relay.mjs`（同一套转发规则）。

## 2026-09-25 · 终端二维码不依赖字形，方像素 PNG 作兜底

- **决定**：`tools/dsh-mobile-pair.mjs` 的终端二维码默认改用**背景色画模块**（每模块 = 两个空格 +
  背景色），不再用「半块字符 ▀」拼；旧画法保留为 `--qr-style half` 供现场对比，并新增
  `--png [路径]` / `--open` 输出**方像素 PNG**（纯标准库自编码，无第三方）。
- **为什么**：`▀` 能否"正好占满上半格"取决于字体——用户机器上实测同一段渲染在该 cmd 窗口里呈
  横条纹（用户原话"乱码"），而背景色画法**不经过任何字形**；PNG 更进一步，连"字符格子是不是
  1:2"都不依赖。
- **否决**：① 在工具里调 Win32 改控制台字体（Node 无 FFI、只对老 conhost 有效、只能从已装字体里挑，
  且会擅自改用户环境）；② 只把"换字体"写进文档（每台机器各改一次、换机复发）。
- 证据：`tools/qr-png.mjs`、`tools/dsh-mobile-pair.mjs`；改动前后对照与 zxing 解码验证见
