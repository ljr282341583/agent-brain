...
- **无半成品。** 桌面端 **v0.3.8** 已发布（提交 `d0a267b` + 文档 `9100512`；CI 全绿 7 分 20 秒、
...
10. ~~项目工作区存在未提交、非本会话所做的改动（"手机访问"半成品）~~ → **已作废（2026-09-25 A机 第二次收尾）**：
    经文件时间戳核对，那批文件是**本会话 agent 自己的中间态**（16:06–16:10），前一会话 16:07 抓到的
    `stopMobileAccess()` 未定义是"写一半"的真缺陷、写完即已定义；现已实测并随 **v0.3.8** 发布。
    核对过程与验证清单见 `JOURNAL\2026-09-25-A机.md` 的「第二次收尾」。**提交纪律照旧**：
    只 `git add` 自己改的文件，不要 `git add -A`。
...
- **更新缓存会留 260 MB 残留（2026-09-25 清过一次）**：`%LOCALAPPDATA%\ds-harness-desktop-updater\`
  ...
  `Setup/Au_/Un_A` 进程在跑。
- **本机 A机 用 agent 的受限环境启动 Electron 需 `--no-sandbox`（2026-09-25 新增）**：不加会秒退，
  退出码 `0x80000003`（`STATUS_BREAKPOINT`，沙箱初始化失败的典型码）；用 Node `spawn` 还是
  PowerShell `Start-Process` 都一样。**这是执行环境限制，与产物无关**——用户机器上的安装版正常。
  连带：`npm run build:dir` 里带的打包冒烟在本机 [3] 会 `EPERM: mkdtemp`、[4] 会撞同一个退出码，
  **报红不代表产物坏**；CI 侧的完整 `npm run build` 已全绿。
- **窗口菜单栏默认藏着**：主窗口沿用 `autoHideMenuBar: true`，**按 `Alt`** 才唤出
  「文件 / 视图 / 手机访问」。用户看过成品后拍板**保持现状**，日常入口用**托盘右键**。
- **尾网 / 内网地址不进笔记、也不进提交信息（2026-09-25 踩过一次）**：本轮在公开仓库的
  commit message 里写了实测监听的尾网 IP。该地址只在 Tailscale 网内可达、无独立利用价值，
  但按红线应脱敏：笔记里一律写"尾网地址"，需要示例时用占位符。
  **用户 2026-09-25 拍板：该笔提交不处理、不重写历史**（改写需强推，会让两台机器的克隆都得重拉，
  风险大于收益）；今后提交信息同样按脱敏口径写。
